...
| `secrets.rs` | Podman secrets via `podman secret ls/create/inspect/rm` (the app's third shell-out). `set` pipes values on stdin (never argv, never a temp file, never logged); the only read-back is `reveal` (`inspect --showsecret`), reached solely via the Value column's eye toggle (`frontend/secrets.js`) -- a CSRF-checked `fetch` POST to `/secrets/reveal` answered as `no-store` plain text and set as `textContent`, logged by name only. Set/Replace use one shared `<dialog>` filled from the clicked button's data attributes, so each table row stays a single line. Never render a value into a page otherwise. Saving (checkbox) or `POST /secrets/restart` restarts the consumers that are running *or failed* (stopped ones are left alone); the per-unit outcome travels to the redirected GET as a one-shot session flash, not a query string. `valid_name` (podman's rule, no leading `-`) gates every name passed to a command. Which units *use* a secret is pure `quadlet::refs` (`secret_refs` / `secret_consumers` / `missing_secrets` -- `.container` `Secret=` only; a `.build`'s `Secret=` reads a host file, not the store). Surfaced on `/secrets` (`handlers::secrets`; delete refused while referenced), the container detail page, each Git Sync card (advisory "missing" badge, never affects `SyncState`), each list-table row (`core::missing_refs` → `ListContext.missing`, an "N missing" badge shared with host variables), and the editor insert panel (lazy `GET /secrets/chips`). |
| `hostenv.rs` | The user manager's `${NAME}` environment. Owns `~/.config/environment.d/50-sooth.conf`; reads other `*.conf` there read-only. Which `${NAME}`s a unit interpolates is pure `quadlet::refs` (`env_refs` / `missing_env` -- any section, any kind, `$$` skipped, minus the unit's own `[Service] Environment=`), checked against the manager's *live* environment (`systemd.environment()`), not the files. `core::RefStores` fetches that and the podman secret list once per request (each only when something references it; `None` = unknown = nothing flagged). Missing ones are flagged on every detail page (`DetailCtx.host_vars`), each Git Sync card, and each list-table row; the links go to `/environment?name=X`, which prefills the add form. |
| `journal.rs` | Shells out to `journalctl --user -u <service> -o json` for the log tail + live follow, parsed into `LogLine`s (`parse_json_line`) that carry each entry's systemd invocation ID (`_SYSTEMD_INVOCATION_ID`, or `USER_INVOCATION_ID` for the manager's own Starting/Stopped messages). `templates::logs` groups lines into runs by it: earlier runs get `.log-old` + a divider, and the "Latest run only" toggle (default on) hides them via CSS; `frontend/logs.js` applies the same marking to live lines. Live lines are `log_line` HTML fragments over SSE, passed through `handlers::logs::normalize_newlines` (`\r`/`\r\n` → `\n`, as the HTML parser does to the initial page) so live and reloaded lines render alike. (`quadlet::gitsync::git` is the app's other shell-out, to `git`.) |
| `imageinfo.rs` | `podman image inspect` for an image's `EXPOSE` ports, and `podman pull` (another shell-out). Inspect is best-effort: any failure (image not pulled, podman missing, timeout) is `None`. A pod member whose image couldn't be inspected (and isn't a `.build`) gets a "Pull image" button: `POST /ports/pull` takes the member's `file_name` (the image is resolved from its quadlet, never from the request), pulls, and answers with the re-rendered `<tbody>` -- a failed pull is a 200 with podman's message inline under that member, since htmx drops non-2xx bodies. Used only by the Ports screen: a pod-published port is shown against the pod member(s) that serve its container port (`quadlet::ports::serves` over the member's `ExposePort=` + its image's `EXPOSE`, image resolved through `.image`/`.build` quadlets by `refs::container_image`), falling back to every member when none declares it. |
| `web/routes.rs` | Router assembly. `mount_unit_routes` registers the shared per-unit routes (detail/actions/start/…/edit/delete/**move**/logs) at **six** prefixes: `/containers /pods /volumes /networks /images /units`. Plus the group-directory routes (`handlers::groups` → `core::*group*`): `POST /groups` (`mkdir` an empty group, or a subgroup with a `parent` field), `/groups/move` (re-parent), `/groups/rename` (rename the leaf), `/groups/delete` (rmdir — refused while any unit lives under it). And the Git Sync routes (`handlers::gitsync` → `GitSyncManager`): `GET /git-sync` (page) + `POST` (add), `/git-sync/rows` (status fragment), `/git-sync/{sync,force,delete}` — each takes its target `group` from the POST body, not a `{group}` path segment, since a nested group's `/` can't live in one. The self-update routes (`handlers::selfupdate` → `SelfUpdateManager`) live under `/settings/self-update` instead of a dedicated page (there's exactly one target, unlike git-sync's arbitrary-many groups): `GET`/`POST` (card fragment + save), `/self-update/check`, `/self-update/download`. Installing a downloaded update posts to the existing `/settings/restart`, not a self-update-specific route. The Secrets routes (`handlers::secrets`): `GET`/`POST /secrets` (page + set/replace, optional `restart`), `/secrets/delete`, `/secrets/restart`, `/secrets/reveal` (plain-text value, `no-store`), and `GET /secrets/chips` (editor insert-panel fragment) -- each takes the secret `name` from the POST body. |
...
- **File name vs on-disk path.** A unit may sit in a group subdirectory, so
  ...
  *bare file name* is still the unique key (podman requires it unique
  ...
  then respawns it under the new key. `templates::list::ListContext` carries
